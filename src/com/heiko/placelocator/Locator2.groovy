...
import com.heiko.placelocator.search2.Impl.TargetSearchHistory
import com.heiko.placelocator.search2.PlaceSearcherFactory
...
try {
    ...
    while (!processor.isSearchFinished(history)) {
        ...
        history.add(new Places(results, config.excludedTypes, config.initialLat, config.initialLng), radius)
    }
